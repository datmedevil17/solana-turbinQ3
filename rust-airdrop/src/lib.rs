...
mod tests {
    ...
    fn transfer_sol() {
        ...
        // Simple message signing verification (optional)
        let message_bytes = b"I verify my Solana Keypair!";
        let _sig = keypair.sign_message(message_bytes);
        println!("Keypair loaded successfully: {}", keypair.pubkey());
        ...
        let balance = match rpc_client.get_balance(&keypair.pubkey()) {
            Ok(balance) => {
                ...
                if balance < 1_000_000 {
                    println!("Insufficient balance for transfer. Please run airdrop first.");
                    return;
                }
                balance
            }
        ...
        let to_pubkey = Pubkey::from_str("EJTYLSSiZ33m68cjWp9D6xd4JC5pDr3R87uJEPt6au1b").unwrap();
        ...
        match rpc_client.send_and_confirm_transaction(&transaction) {
            Ok(signature) => {
                ...
                println!(
                    "Check your TX here: https://explorer.solana.com/tx/{}/?cluster=devnet",
                    signature
                );
            }
            Err(err) => {
